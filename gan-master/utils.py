...
def lr_images(images_real, downscale):
    ...
    for img in range(len(images_real)):
        ...
        # 이미지 크기를 줄였다가 다시 키우는 방식으로 변경해서, 해상도는 낮추고 이미지 사이즈는 동일하게 맞춤
        im = imresize(images_real[img], [images_real[img].shape[0] // downscale, images_real[img].shape[1] // downscale], interp='bicubic', mode=None)
        im = imresize(im, [im.shape[0] * downscale, im.shape[1] * downscale], interp='bicubic', mode=None)
        images.append(im)
...
def plot_generated_images(output_dir, epoch, generator, generator_diff, x_test_hr, x_test_lr, title, dim=(1, 4), figsize=(20, 5)):
    ...
    gen_img_diff = generator_diff.predict(image_batch_lr)
    generated_image = denormalize(gen_img)
    generated_image_diff = denormalize(gen_img_diff)
    ...
    plt.imshow(generated_image_diff[value], interpolation='nearest')
    plt.title(title[1])
    plt.axis('off')
