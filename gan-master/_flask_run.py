...
# if not os.path.isdir('/_generate_image'):
#     os.makedirs('/_generate_image')

# app = Flask(__name__)
# app = Flask(__name__, static_url_path = "/_generate_image", static_folder = "_generate_image")
...
#         return name + " Hello"
#     return render_template("index_ex_1.html")
...
#     return render_template('index_ex_2.html')
...
def generate_test(filename):
    import shutil
    shutil.copy('_uploads/' + filename, '_generate_image/' + filename)
    return filename


def generate_no_split(filename):
    K.clear_session()
    ...
    image = data.imread('_uploads/' + filename)
    ...
    model = load_model('model/gen_model100.h5', custom_objects={'vgg_loss': loss.vgg_loss})
    print('load_model...')
...
def generate(filename):
    ...
    # image = cv2.resize(image, (image_shape[0] * w_count, image_shape[1] * h_count), interpolation=cv2.INTER_AREA)
    ...
    # 이미지 분할해서 여러 개로 가져오기
    # images = image_split(image, image_shape)
    images = image_split_overlap(image, image_shape, edge_size)
    image = utils.hr_images(images)
    print('image shape: ', image.shape)

    image = utils.normalize(image)
    model = load_model('model/gen_model100_t.h5', custom_objects={'vgg_loss': loss.vgg_loss})
    # model = multi_gpu_model(model, gpus=None, cpu_merge=True)
    print('load_model...')
...
# @app.route('/download_gen', methods=['GET', 'POST'])
# def download_gen():
#     file_name = 'small.png'
#     return send_from_directory('_generate_image', file_name, as_attachment=True)
#     # file_name = '_generate_image/width.png'
#     # return send_file(file_name, mimetype='image/png', as_attachment=True)
#     #                   attachment_filename='generate.png' 를 추가하면 해당 파일명으로 다운됨
